    ...
    return (
        <div className="flex flex-col justify-evenly items-center shadow-xl rounded-lg w-[600px] h-[600px] gap-5">
            ...
            <div className='flex flex-col gap-5 text-center text-lg font-bold'>
                <span>Acertaste</span>
                <span className='font-blanck bg-gradient-to-r from-purple-500 to-pink-500 bg-clip-text text-transparent text-6xl animate-pulse'>
                    {((score / questionsFiltered.length) * 100).toFixed(0)} %
                </span>
                <span> de las preguntas de ({score} de {questionsFiltered.length})</span>
            </div>
            <button 
